...
//         // Map API ingredientId (number) to state id (string)
...
//             const matchedIngredient = fetchedData.find(
//               (item: { ingredientId: number; quantity: number }) => idMap[item.ingredientId] === ingredient.id
//             )
//             if (matchedIngredient) {
//               return {
//                 ...ingredient,
//                 inMug: matchedIngredient.quantity,
//                 available: matchedIngredient.quantity, // Assuming available is the quantity in inventory
//               }
//             } else {
//               return {
//                 ...ingredient,
//                 inMug: 0,
//                 available: 0, // Set to 0 if not present in API data
//               }
//             }
//           })
//         )
...
//     setIngredients((prev) =>
//       prev.map((ing) => (ing.id === id && ing.available > 0 ? { ...ing, inMug: ing.inMug + 1, available: ing.available - 1 } : ing))
//     )
//   }
...
//     if (!redeemable || !coffeeType) {
//       setError('Please select a coffee type and ensure enough ingredients are added')
//       return
//     }
//     setRedeeming(true)
//     setError(null)
...
//     try {
//       await axios.post('/api/coffee', { type: coffeeType })

//       // Refresh ingredients by re-fetching from /api/ingredient
//       const response = await axios.get('/api/ingredient', {
//         params: { userId: 1 },
//       })
//       const fetchedData = response.data.data
...
//               ...ingredient,
//               inMug: matchedIngredient.quantity,
//               available: matchedIngredient.quantity,
//             }
//           } else {
//             return {
//               ...ingredient,
...
//       setCoffeeType(null) // Reset coffee type selection
//     } catch (error: any) {
...
//             className={`py-2 px-4 rounded-lg ${coffeeType === 'Latte' ? 'bg-amber-500 text-white' : 'bg-gray-200 text-gray-800'} transition-all`}
//           >
//             Latte
//           </button>
//           <button
//             onClick={() => setCoffeeType('Mocha')}
//             className={`py-2 px-4 rounded-lg ${coffeeType === 'Mocha' ? 'bg-amber-500 text-white' : 'bg-gray-200 text-gray-800'} transition-all`}
//           >
//             Mocha
//           </button>
//           <button
...
//             {ingredients.every((i) => i.inMug === 0) && !coffeeType && (
//               <p className="text-sm text-gray-500 italic">Drag ingredients to the mug to start customizing your coffee.</p>
//             )}
...
const Dashboard: React.FC = () => {
        ...
        setIngredients((prevIngredients) =>
          prevIngredients.map((ingredient) => {
            ...
            return {
              ...
              inMug: 0, // No ingredients in mug by default
            }
          })
        )
      } catch (error) {
  ...
  useEffect(() => {
    if (coffeeType) {
      ...
      const hasSufficientIngredients = requiredIngredients.every((ingId) => {
        const ingredient = ingredients.find((i) => i.id === ingId)
        return ingredient && ingredient.available >= 1
      })
      setRedeemable(hasSufficientIngredients)
    } else {
      setRedeemable(false)
    }
